...
if args.mode == 'train':
    ...
    # データセットの準備
    #
    encoder_input_id = np.zeros((len(encoder_input_texts), max_input_seq_length), dtype='uint32')
    decoder_input_id = np.zeros((len(decoder_input_texts), max_target_seq_length), dtype='uint32')
    #decoder_target_texts = np.zeros((len(decoder_input_texts), max_input_seq_length), dtype='object')
    decoder_target_text = []
    ...
    for i,(encoder_seq, decoder_seq) in enumerate(zip(encoder_input_texts, decoder_input_texts)):
        for j, (encoder_word, decoder_word) in enumerate(zip(encoder_seq, decoder_seq)):
            encoder_input_id[i,j] = word_index[encoder_word]
            decoder_input_id[i,j] = word_index[decoder_word]  
            if j > 0:
                #decoder_target_texts[i,j-1] = decoder_word
                decoder_target_text.append(decoder_word)
        decoder_target_texts.append(decoder_target_text)
        decoder_target_text= []
    
    # encoderr_inputとdecoderr_inputだけ先にtrainとvalに分割
    encoder_id_train, encoder_id_val = split_train_val_np(encoder_input_id, train_val_rate)
    decoder_id_train, decoder_id_val = split_train_val_np(decoder_input_id, train_val_rate)
    #target_texts_train, target_texts_val = split_train_val_list(decoder_target_texts, train_val_rate)
    target_texts_train, target_texts_val = train_test_split(decoder_target_texts, train_size=0.8)
    ...
    for j in range(0, epochs) :
        ...
        for i in range(0, n_batch):
            ...
            target_train_batch = target_texts_train[start_train:end_train:]
            
            encoder_val_batch = encoder_id_val[start_val:end_val,:]
            decoder_val_batch = decoder_id_val[start_val:end_val,:]
            target_val_batch = target_texts_val[start_val:end_val:]
            ...
            for i, target_seq in enumerate(target_train_batch):
                for j, target_word in enumerate(target_seq):

                    target_onehot_train_batch[i, j, word_index[target_word]] = 1 
            
            for i, target_seq in enumerate(target_val_batch):
                for j, target_word in enumerate(target_seq):
                    target_onehot_val_batch[i, j, word_index[target_word]] = 1 
